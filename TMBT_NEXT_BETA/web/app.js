...
async function openTradeReplay(hid,phase="entry"){
 try{
  ...
  state.snapshot=null;state.tradeReplay=d;state.snapshotReturnView="outcomes";state.mode="trade_replay";
  ...
  $("#backOutcome").style.display="";$("#backOutcome").textContent="← Outcomes";
...
function backOutcomes(){const target=state.snapshotReturnView||"outcomes";state.tradeReplay=null;state.snapshot=null;state.mode="live";$("#snapshotBanner").classList.remove("show");$("#backOutcome").style.display="none";$("#backOutcome").textContent="← Outcomes";$("#snapshotModeLabel").textContent="FROZEN SNAPSHOT";setWorkspaceView(target);if(target==="outcomes")refreshOutcomes();if(target==="archive")refreshArchive()}
...
function renderReview(){
 ...
 const reviewHtml='<div class="review-grid">'+ready.map(p=>{
  ...
  return `<article class="review-card">
   ...
   ${perf.full_history?.available?`<div class="full-history-note">Gesamter verfügbarer Datenzeitraum · Reporting-only, nicht für Optimizer oder Live-Gate verwendet.</div>${performanceDetail(perf.full_history,"Gesamter Datenzeitraum",true)}`:""}
...
function renderResearch(){
 ...
 const reportHtml=p=>{
  const x=p?.last_analysis||{},dev=x.development_summary||{},val=x.validation_summary||{},good=x.good||[],bad=x.bad||[],next=x.next_steps||[],ov=x.selected_overrides||{},perf=x.performance||{},sp=x.data_splits||{},prov=x.provenance||{},pipe=x.pipeline_contract||{};
  if(!x.profile)return"";
  const splitLine=(sp.development_start||sp.validation_start)?`<div class="research-split-line"><b>Daten:</b> ${esc(sp.full_start||sp.development_start||"—")} → ${esc(sp.full_end||sp.holdout_end||sp.validation_end||"—")} · Development ${esc(sp.development_start||"—")} → ${esc(sp.development_end||"—")} · Validation ${esc(sp.validation_start||"—")} → ${esc(sp.validation_end||"—")}${sp.holdout_start?" · Holdout "+esc(sp.holdout_start)+" → "+esc(sp.holdout_end||"—"):""}</div>`:"";
  const provenanceLine=prov.source_status?`<div class="research-split-line"><b>Quellenstatus:</b> ${esc(prov.source_status)} · ${esc(prov.evidence_label||prov.evidence_class||"—")} · ${esc(prov.source_family||"—")}${prov.variant_status?` · ${esc(prov.variant_status)}`:""}${(prov.non_core_assumptions||[]).length?` · TMBT/Later: ${esc((prov.non_core_assumptions||[]).join(", "))}`:""}</div>`:"";
  const pipelineLine=pipe.pipeline_version?`<div class="research-split-line"><b>Regel-Pipeline:</b> ${esc(pipe.pipeline_version)} · Context ${esc(pipe.context?.status||"—")} → Event ${esc(pipe.event?.status||"—")} → Setup ${esc(pipe.setup?.status||"—")} → Execution ${esc(pipe.execution?.status||"—")} → Risk ${esc(pipe.risk?.status||"—")}</div>`:"";
  return`<details class="research-card"><summary><b>Bericht · ${esc(x.label||x.profile)}</b> <span class="pill ${String(x.verdict||"idle").toLowerCase()}">${esc(x.verdict||"—")}</span></summary>${splitLine}${provenanceLine}${pipelineLine}<div class="model-row"><small>Development: Trades ${dev.trades??"—"} · Exp ${fmt(dev.expectancy_r,3)}R · PF ${fmt(dev.profit_factor_r,2)} · DD ${fmt(dev.max_drawdown_r,1)}R</small><small>Validation: Trades ${val.trades??"—"} · Exp ${fmt(val.expectancy_r,3)}R · PF ${fmt(val.profit_factor_r,2)} · DD ${fmt(val.max_drawdown_r,1)}R</small></div>${validationGateHtml(x)}${perf.full_history?.available?`<div class="full-history-note">Gesamter verfügbarer Datenzeitraum · Reporting-only, nicht für Optimizer oder Live-Gate verwendet.</div>${performanceDetail(perf.full_history,"Gesamter Datenzeitraum",false)}`:""}${performanceDetail(perf.validation,"Validation",false)}<details class="optimization-report"><summary><b>Optimierungsverlauf</b><span>${(x.optimization_steps||[]).length} Schritte</span></summary>${optimizationHistory(x)}</details><h4>Gut</h4><ul>${good.map(v=>`<li class="good">${esc(v)}</li>`).join("")||"<li>—</li>"}</ul><h4>Schlecht / Risiken</h4><ul>${bad.map(v=>`<li class="bad">${esc(v)}</li>`).join("")||"<li>—</li>"}</ul><h4>Nächster Schritt</h4><ul>${next.map(v=>`<li>${esc(v)}</li>`).join("")||"<li>—</li>"}</ul><details><summary>Gewählte Parameter</summary><pre>${esc(JSON.stringify(ov,null,2))}</pre></details></details>`;
 };
 ...
 const kb=state.ictCoreCoverage||{},kbComp=kb.completion||{},kbMachine=kb.machine_status_counts||{},kbEvidence=kb.evidence_counts||{},kbr=state.ictCoreSearchResults||{};
 const kbSearchText=state.ictCoreQuery?JSON.stringify(kbr,null,2):"";
 const knowledge=`<div class="research-card research-autopilot"><div class="model-row"><div><h4>ICT Core Knowledge Base</h4><small>${esc(kb.knowledge_version||"wird geladen")}</small></div><span class="pill ${kb.fully_source_certified?"signal":"armed"}">${kb.lecture_count??0}/${kb.expected_lecture_count??115} LEKTIONEN · ${kb.fully_source_certified?"SOURCE CERTIFIED":"VISUAL AUDIT OFFEN"}</span></div><div class="model-row"><small>Rule groups: ${kb.rule_catalog_count??"—"} · gemappte Lektionen ${kb.rule_mapped_lecture_count??"—"}/115 · Executable primitives: ${kb.executable_rule_count??"—"}</small><small>A/B/C/D: ${kbEvidence.A??0}/${kbEvidence.B??0}/${kbEvidence.C??0}/${kbEvidence.D??0}</small></div><div class="model-row"><small>Machine: READY ${kbMachine.READY??0} · PARTIAL ${kbMachine.PARTIAL??0} · REFERENCE ${kbMachine.REFERENCE??0} · VISUAL ${kbMachine.VISUAL??0}</small><small>Visual audit: ${esc(kbComp.visual_geometry_audit||"—")} · Locked ${kb.visual_locked_rule_count??0}</small></div><div class="model-row"><input id="ictCoreSearch" value="${esc(state.ictCoreQuery)}" placeholder="Core Content durchsuchen: FVG, CBDR, Order Block …"><button id="ictCoreSearchBtn" class="ghost">Suchen</button></div>${kbSearchText?`<details open><summary><b>Suchergebnis · ${esc(state.ictCoreQuery)}</b></summary><pre>${esc(kbSearchText)}</pre></details>`:""}${(kb.visual_locked_rules||[]).length?`<details><summary><b>Offene visuelle Zertifizierung</b> <small>${kb.visual_locked_rule_count||0} Regelgruppen</small></summary><pre>${esc((kb.visual_locked_rules||[]).join("\n"))}</pre></details>`:""}<p class="muted">Die Wissensabdeckung ist 115/115 vollständig. Vollständige Quellen-Zertifizierung bleibt offen, bis alle chartabhängigen Geometrien visuell bestätigt oder dauerhaft als nicht-deterministisch gesperrt sind.</p></div>`;
 ...
 $("#ictCoreSearchBtn")?.addEventListener("click",searchIctCoreKnowledge);
 $("#ictCoreSearch")?.addEventListener("keydown",e=>{if(e.key==="Enter")searchIctCoreKnowledge()});
}
