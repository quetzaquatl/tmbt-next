...
function renderResearch(){
 ...
 const knowledge=`<div class="research-card research-autopilot"><div class="model-row"><div><h4>ICT Core Knowledge Base</h4><small>${esc(kb.knowledge_version||"wird geladen")}</small></div><span class="pill ${kb.fully_rule_audited?"signal":"armed"}">${kb.lecture_count??0}/${kb.expected_lecture_count??115} LEKTIONEN · ${kb.fully_rule_audited?"RULE AUDIT COMPLETE":"SOURCE AUDIT OFFEN"}</span></div><div class="model-row"><small>Rule groups: ${kb.rule_catalog_count??"—"} · gemappte Lektionen ${kb.rule_mapped_lecture_count??"—"}/115 · Executable primitives: ${kb.executable_rule_count??"—"}</small><small>A/B/C/D: ${kbEvidence.A??0}/${kbEvidence.B??0}/${kbEvidence.C??0}/${kbEvidence.D??0}</small></div><div class="model-row"><small>Machine: READY ${kbMachine.READY??0} · PARTIAL ${kbMachine.PARTIAL??0} · REFERENCE ${kbMachine.REFERENCE??0} · VISUAL ${kbMachine.VISUAL??0}</small><small>Source audit: ${esc(kbComp.source_rule_audit||"—")} · Locked ${kb.visual_locked_rule_count??0}</small></div><div class="model-row"><input id="ictCoreSearch" value="${esc(state.ictCoreQuery)}" placeholder="Core Content durchsuchen: FVG, CBDR, Order Block …"><button id="ictCoreSearchBtn" class="ghost">Suchen</button></div>${kbSearchText?`<details open><summary><b>Suchergebnis · ${esc(state.ictCoreQuery)}</b></summary><pre>${esc(kbSearchText)}</pre></details>`:""}${(kb.visual_locked_rules||[]).length?`<details><summary><b>Offene visuelle Zertifizierung</b> <small>${kb.visual_locked_rule_count||0} Regelgruppen</small></summary><pre>${esc((kb.visual_locked_rules||[]).join("\n"))}</pre></details>`:""}<p class="muted">115/115 Wissens- und Regelquellen sind auditiert. Ein manuelles Frame-by-Frame-Abspielen aller 53+ Videostunden wird nicht behauptet; PARTIAL/REFERENCE-Regeln bleiben trotzdem aus der Auto-Execution gesperrt.</p></div>`;
