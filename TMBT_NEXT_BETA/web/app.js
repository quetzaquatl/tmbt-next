...
function renderResearch(){
 ...
 const reportHtml=p=>{
  ...
  const provenanceLine=prov.source_status?`<div class="research-split-line"><b>Quellenstatus:</b> ${esc(prov.source_status)} · ${esc(prov.evidence_label||prov.evidence_class||"—")} · ${esc(prov.source_family||"—")}${(prov.non_core_assumptions||[]).length?` · TMBT/Later: ${esc((prov.non_core_assumptions||[]).join(", "))}`:""}</div>`:"";
