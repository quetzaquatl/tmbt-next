...
def _old_processes() -> list[dict[str, Any]]:
    if os.name != "nt":
        return []
    # Only count actual legacy runtime processes. The verification helper
    # itself launches PowerShell with OLD_ROOT in its -Command argument, so a
    # broad CommandLine match falsely counted its own PowerShell process.
    old = str(OLD_ROOT).replace("'", "''")
    ps = (
        "$me=$PID; "
        "Get-CimInstance Win32_Process | "
        f"Where-Object {{ $_.ProcessId -ne $me -and $_.CommandLine -like '*{old}*' -and "
